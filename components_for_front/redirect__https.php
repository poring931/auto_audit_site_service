...
if ($_GET['url'] !=''){
    ...
    if (substr_count($url,'https')){ // првоеряем что ввели: http\https
        $url = str_replace('https','http',$url);  //меняем гет урл на нттп и смотрим ответ сервера
        ...
        if ($httpCode!= '200' && $httpCode!= '404'){
           echo 'есть (код - ' .$httpCode . ' ' . $url . ')';
            ...
            echo ' <script>array_audit.redirecthttp = "есть"</script> ';
        } else {
            echo 'редирект отсутствует';
            echo '
            <script>array_audit.redirecthttp = "редирект отсутствует"</script>
            ';
    ...
    echo ' <script>array_audit.date = "' . time() . '"</script> '; 
}