...
if (isset($_POST['url'])){
    $get_url =htmlspecialchars($_POST['url']);
} else {
    $get_url = 'https://www.lightup.su/'; // получаем урл через гет запрос
}
...
foreach ($doc->find('script') as $script) {
       ...
       if (stripos($script->nodeValue, '@context') !== false) { //находим jsonld Разметку
              if (stripos($script->nodeValue, '/schema.org') !== false) { //находим jsonld Разметку
                     $AUDIT_SITE['schema_org']["is_set"] = 'is_set - true'; }
              if (stripos($script->nodeValue, 'Organization') !== false) { //находим jsonld Разметку
                     $AUDIT_SITE['schema_org']["Organization"] = 'Organization - true';
              } else {
                      $AUDIT_SITE['schema_org']["Organization"] = 'Organization - false';
              }
              if (stripos($script->nodeValue, 'logo') !== false) { //находим jsonld Разметку
                     $AUDIT_SITE['schema_org']["logo"] = 'logo - true';
              } else {
                      $AUDIT_SITE['schema_org']["logo"] = 'logo - false';
              }
              if (stripos($script->nodeValue, 'contactPoint') !== false) { //находим jsonld Разметку
                     $AUDIT_SITE['schema_org']["contactPoint"] = 'contactPoint - true';
              } else {
                      $AUDIT_SITE['schema_org']["contactPoint"] = 'contactPoint - false';
              }
              if (stripos($script->nodeValue, 'BreadcrumbList') !== false) { //находим jsonld Разметку
                     $AUDIT_SITE['schema_org']["BreadcrumbList"] = 'BreadcrumbList - true';
              } else {
                      $AUDIT_SITE['schema_org']["BreadcrumbList"] = 'BreadcrumbList - false';
              }
              if (stripos($script->nodeValue, 'sameAs') !== false) { //соц сети
                     $AUDIT_SITE['schema_org']["sameAs"] = 'sameAs - true';
              } else {
                      $AUDIT_SITE['schema_org']["sameAs"] = 'sameAs - false';
              }
              if (stripos($script->nodeValue, 'LocalBusiness') !== false) { 
                     $AUDIT_SITE['schema_org']["LocalBusiness"] = 'LocalBusiness - true';
              } else {
                      $AUDIT_SITE['schema_org']["LocalBusiness"] = 'LocalBusiness - false';
              }
              if (stripos($script->nodeValue, 'WebSite') !== false) { //
                     $AUDIT_SITE['schema_org']["WebSite"] = 'WebSite - true';
              } else {
                      $AUDIT_SITE['schema_org']["WebSite"] = 'WebSite - false';
              }
              if (stripos($script->nodeValue, 'Product') !== false) { //
                     $AUDIT_SITE['schema_org']["Product"] = 'Product - true';
              } else {
                      $AUDIT_SITE['schema_org']["Product"] = 'Product - false';
              }
       }
}
if ($AUDIT_SITE['schema_org'] != 'true'){ //доп проверка, если нет в скриптах JSONld , то чекаем все обьекты страницы
       foreach($doc->find('*') as $element){ //берем все ссылки из шаки, которые в себе держат урл домена. Чтоб потом по ним пробежаться
              if (stripos(pq($element)->attr("itemtype"), '/schema.org') !== false) { $AUDIT_SITE['schema_org']['is_set'] = 'is_set - true'; } else {
                     $AUDIT_SITE['schema_org']["is_set"] = 'is_set - false'; 
              }
              if (stripos(pq($element)->attr("itemtype"), '/schema.org/BreadcrumbList') !== false) { $AUDIT_SITE['schema_org']["BreadcrumbList"] = 'BreadcrumbList - true'; } else {
                      $AUDIT_SITE['schema_org']["BreadcrumbList"] = 'BreadcrumbList - false'; 
              }
              if (stripos(pq($element)->attr("itemtype"), '/schema.org/WPFooter') !== false) { $AUDIT_SITE['schema_org']["WPFooter"] = 'true - true'; } else {
                     $AUDIT_SITE['schema_org']["WPFooter"] = 'WPFooter - false';    
              }
              if (stripos(pq($element)->attr("itemtype"), '/schema.org/WPHeader') !== false) { $AUDIT_SITE['schema_org']["WPHeader"] = 'WPHeader - true'; } else {
                     $AUDIT_SITE['schema_org']["WPHeader"] = 'WPHeader - false'; 
              }
              if (stripos(pq($element)->attr("itemtype"), '/schema.org/SiteNavigationElement') !== false) { $AUDIT_SITE['schema_org']["SiteNavigationElement"] = 'true'; } else {
                     $AUDIT_SITE['schema_org']["SiteNavigationElement"] = 'SiteNavigationElement - false'; 
              }
              if (stripos(pq($element)->attr("itemtype"), '/schema.org/Product') !== false) { $AUDIT_SITE['schema_org']["Product"] = 'true'; } else {
                      $AUDIT_SITE['schema_org']["Product"] = 'Product - false'; 
              }
       }
}
...
//     {
        foreach ($url_crawlArra as $key => $value) { //загружает каждую страницу, вошедшую в этот массив
              ...
              // -------------------------Поиск битых ссылок на главной  НАЧАЛО-------------------------------------------------------------------
              ...
              // -------------------------Поиск битых ссылок на главной  конец-------------------------------------------------------------------


               // -------------------------поиск H1 на первых 10 ссылках  НАЧАЛО-------------------------------------------------------------------
               // поиск H1 на первых 10 ссылках  НАЧАЛО
               //тут выпадала ошибка. @ - отключил пока ее  "Warning: file_get_contents(https://lightup.su/top): failed to open stream: HTTP request failed! HTTP/1.1 404 Not Found in /home/s/seoins23/im-pulse33.ru/public_html/phpQuery__crawling/test__indexable_pages.php on line 235"
        ...
        // -------------------------Поиск внешних ссылок на первых 10 ссылках  НАЧАЛО-------------------------------------------------------------------
               ...
               if (mb_strlen($data['description']) < 70){$AUDIT_SITE['page_with_short__description'][] .= mb_strlen ($data['description']).','.$value;}
               if (mb_strlen($data['description']) < 155){$AUDIT_SITE['page_with_long__description'][] .= mb_strlen ($data['description']).','.$value;}
       ...
       // --------------------- поиск скриптов схемы орг, метркики  -----------------------------------------------------------------
       if ($AUDIT_SITE['schema_org']["BreadcrumbList"] == '') {
              foreach ($doc__inner_first_10__links['{$count_inter}']->find('*')  as $element) {
                     if (stripos(pq($element)->attr("itemtype"), '/schema.org/BreadcrumbList') !== false) {
                            $AUDIT_SITE['schema_org']["BreadcrumbList"] = 'true';
                     }
                     if (stripos(pq($element)->attr("itemtype"), '/schema.org/Product') !== false) {
                            $AUDIT_SITE['schema_org']["Product"] = 'true';
                     }
                     if (stripos(pq($element)->attr("itemtype"), '/schema.org/FAQPage') !== false) {
                            $AUDIT_SITE['schema_org']["FAQPage"] = 'true';
                     }
              }
       }

       foreach ($doc__inner_first_10__links['{$count_inter}']->find('script') as $script) {
     
              if (stripos($script->nodeValue, '@context') !== false) { //находим jsonld Разметку

                     if ($AUDIT_SITE['schema_org']["WebSite"] =='' ){
                            if (stripos($script->nodeValue, 'WebSite') !== false) { //
                                   $AUDIT_SITE['schema_org']["WebSite"] = 'true';
                            }
                     }
                     if ($AUDIT_SITE['schema_org']["FAQPage"] =='' ){
                            if (stripos($script->nodeValue, 'FAQPage') !== false) { //
                                   $AUDIT_SITE['schema_org']["FAQPage"] = 'true';
                            }
                     }
                     if ($AUDIT_SITE['schema_org']["Product"] =='' ){
                            if (stripos($script->nodeValue, 'Product') !== false) { //
                                   $AUDIT_SITE['schema_org']["Product"] = 'true';
                            }
                     }
                     if ($AUDIT_SITE['schema_org']["LocalBusiness"] =='' ){
                            if (stripos($script->nodeValue, 'LocalBusiness') !== false) {
                                   $AUDIT_SITE['schema_org']["LocalBusiness"] = 'true';
                            }
                     }
                     if ($AUDIT_SITE['schema_org']["sameAs"] =='' ){
                            if (stripos($script->nodeValue, 'sameAs') !== false) { //соц сети
                                   $AUDIT_SITE['schema_org']["sameAs"] = 'true';
                            }
                     }
                     if ($AUDIT_SITE['schema_org']["BreadcrumbList"] =='' ){
                            if (stripos($script->nodeValue, 'BreadcrumbList') !== false) { 
                                   $AUDIT_SITE['schema_org']["BreadcrumbList"] = 'true';
                            } 
                     }
                     if ($AUDIT_SITE['schema_org']["contactPoint"] =='' ){
                            if (stripos($script->nodeValue, 'contactPoint') !== false) { //находим jsonld Разметку
                                   $AUDIT_SITE['schema_org']["contactPoint"] = 'true';
                            }
                     }
                     if ($AUDIT_SITE['schema_org']["logo"] =='' ){
                            if (stripos($script->nodeValue, 'logo') !== false) { //находим jsonld Разметку
                                   $AUDIT_SITE['schema_org']["logo"] = 'true';
                            }
                     }
                     if ($AUDIT_SITE['schema_org']["Organization"] =='' ){
                            if (stripos($script->nodeValue, 'Organization') !== false) { //находим jsonld Разметку
 ...
 // --------------------- поиск скриптов схемы орг, метркики  -----------------------------------------------------------------
       ...
       phpQuery::unloadDocuments();//PhpQuery очень удобная библиотека, но, к сожалению, слишком тяжелая. Так что после прохода по элементам рекомендуется выгружать документ:
...
if (!empty($duplicates)){
       foreach ($duplicates as  $key => $duble) {
              $AUDIT_SITE['page_copies_description'][$key] = $duble . ' - ' . $key;
       }
} else {
